...
class YySpider(RedisCrawlSpider):
    ...
    start_urls = ['http://www.uy99.com/search/index.aspx?key=2_20_30_-1_0_-1_-1_-1_-1_-1_0_-1_110000_-1_-1_-1_-1_1_-1_-1_-1_-1_-1_-1_-1&p=1']
    ...
    def parse_item(self, response):
        print('='*30)
        from spiders.scrapy.disSpiderPrac.yyuanSpider.yyuanSpider.items import YyuanspiderItem
        item = YyuanspiderItem()
    ...
    def get_place_from(self, response):
        place_from = response.xpath("//div[@id='pbody']//div[@class='baseinfo']//div[@class='ubase']/a/text()").extract()
        if len(place_from):
